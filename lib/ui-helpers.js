...
function changeFrequencyButton(
  ...
  return {
    ...
    body: `Your memorization schedule will be: ${getFutureIntervalSentence(
      numExampleIntervals,
      frequencyPref,
      startingFrom
    )}. <br /><br />Each time you successfully remember, you will move to the next further interval.`
  };
}
...
function memorizationControls(
  ...
  const timeFromNow = daysInFuture
    ? `${daysInFuture} ${daysInFuture === 1 ? "day" : "days"} from today`
  ...
  return [
    ...
    {
      type: "html",
      text: `Each time you successfully remember, reminders will follow this schedule. The next one is ${timeFromNow}.`
    },
    {
      type: "html",
    ...
    ...changeFrequencyButtons(frequencyPref, frequencyOptions, reminderCount)
  ];
}
...
function renderRepeatMessage(gopher) {
  ...
  const messageTable = {
    ...
    "0": "",
    "1":
      "Were you able to remember? We will send 2 more reminders before turning off this memorization.",
    "2":
      "Were you able to remember? We will send 1 more reminders before turning off this memorization.",
    "3":
      "This memorization has been paused. To re-enable it, click 'yes' or 'no' below."
  };
  if (messageTable[howManyRepeatReminders] === undefined) {
    throw new Error(`Unexpected input: ${howManyRepeatReminders}`);
  }
  return { type: "html", text: messageTable[howManyRepeatReminders] };
}
