...
function renderFrequencyIllustration(
  ...
  let daysInFuture = getFutureIntervals(100, frequencyPref, reminderNum).map(
...
function getFutureIntervalSentence(
  ...
  futureIntervalSentence += "etc from from now.";
...
function changeFrequencyButton(
  ...
  return {
    ...
    body: `Your new memorization schedule will be: ${getFutureIntervalSentence(
      numExampleIntervals,
      frequencyPref,
      nextReminderNum
    )}.

    Your next reminder will arrive on the first date of this new schedule.`
  };
}
